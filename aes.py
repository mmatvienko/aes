...
def InvSubBytes(inp):
    ...
    for b in inp:
        new.append(sbox1[b])
    # print("After InvSubBytes: ", [hex(x) for x in new])
    return new
...
def InvShiftRows(inp):
    ...
    inp = transpose(inp)
    # print("After InvShiftRows: ", [hex(x) for x in inp])
    return inp
...
def InvMixColumns(inp):
    p = [0]*len(inp)
    # print("mix colums inp: ", [hex(x) for x in inp])
    inp = transpose(inp)
    for c in range(4):
        # print("c: ", c)
        p[c] = (mul0e[inp[c]]) ^ (mul0b[inp[1*4+c]]) ^ (mul0d[inp[2*4+c]]) ^ (mul9[inp[3*4+c]])
        p[1*4+c] = (mul9[inp[c]]) ^ (mul0e[inp[1*4+c]]) ^ (mul0b[inp[2*4+c]]) ^ (mul0d[inp[3*4+c]])
        p[2*4+c] = (mul0d[inp[c]]) ^ (mul9[inp[1*4+c]]) ^ (mul0e[inp[2*4+c]]) ^ (mul0b[inp[3*4+c]])
        p[3*4+c] = (mul0b[inp[c]]) ^ (mul0d[inp[1*4+c]]) ^ (mul9[inp[2*4+c]]) ^ (mul0e[inp[3*4+c]])
    p = transpose(p)
    # print("After InvMixColumns: ", [hex(x) for x in p])
    return p
...
def AddRoundKey(inp, w):
    ...
    for c in range(4):
        word = w[c]
        p[c] = inp[c] ^ (w[0]>>(8*(3-c))) & 0xFF
        p[1*4+c] = inp[1*4+c] ^ (w[1]>>(8*(3-c))) & 0xFF
        p[2*4+c] = inp[2*4+c] ^ (w[2]>>(8*(3-c))) & 0xFF
        p[3*4+c] = inp[3*4+c] ^ (w[3]>>(8*(3-c))) & 0xFF
...
def InvCipher(inp, w, Nb, Nr):
    inp = AddRoundKey(inp, w[Nr*Nb:(Nr+1)*Nb])
    for round in range(1,Nr)[::-1]:
        inp = InvShiftRows(inp)
        inp = InvSubBytes(inp)
        inp = AddRoundKey(inp, w[round*Nb:(round+1)*Nb])
        inp = InvMixColumns(inp)
        # print how many rounds
    inp = InvShiftRows(inp)
    inp = InvSubBytes(inp)
    inp = AddRoundKey(inp, w[0:Nb]) # is this right?
